...
**Email:** The response daemon sends emails via `gws gmail +send` to your-email@example.com (configurable via `RESPONSE_EMAIL_TO` env var). Subject line format: `[fleet-security] SEVERITY on MACHINE: incident_type`. Throttled to 1 email per machine per 15 minutes.

**Gridwatch Dashboard:** The fleet health dashboard at `http://<broker-ip>:8888` shows real-time machine health scores. Machines turn yellow (degraded, score 5-9) or red (quarantined, score 10+). Run `claude-peers gridwatch` locally to view.
...
5. Check if the token was used:
   ```bash
   # On broker-server (broker), check broker logs
   ssh broker-server "journalctl -u claude-peers-broker --since '1 hour ago' | grep <machine>"
   ```
...
3. Check all machines for activity from that IP:
   ```bash
   for machine in workstation edge-node laptop-1 iot-device workstation-2; do
     echo "=== $machine ==="
...
1. Block the source IP on ALL fleet machines:
   ```bash
   for machine in workstation broker-server edge-node laptop-1 iot-device workstation-2; do
...
- [ ] If broker (broker-server) was compromised: rotate NATS tokens, rotate all fleet tokens
...
| Who | Role | Reach |
|---|---|---|
| the operator | Fleet owner, all access | your-email@example.com |
| partner | HFL partner, backup | Via HFL channels |
...
claude-peers sim-attack brute-force --target=edge-node

# Run all scenarios
claude-peers sim-attack --all --target=edge-node
...
Default target is `edge-node`. The command will refuse to target `broker-server` (the broker) without explicit confirmation.
