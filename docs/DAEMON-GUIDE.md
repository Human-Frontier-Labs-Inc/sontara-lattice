...
INPUT broker_url DEFAULT "http://<broker-ip>:7899"
...
health=$(curl -sf -H "Authorization: Bearer $(cat ~/.config/claude-peers/token.jwt)" \
  http://<broker-ip>:7899/machine-health 2>/dev/null)
...
- **pr-helper**: Refuses to push code if broker-server is quarantined.
...
| pr-helper | 15m | Keep PRs mergeable across GitHub orgs (human-frontier-lab, williavs, the operatorV3) |
