...
function normalizeMachine(name) {
  ...
  // Check if the name directly matches a known machine.
  if (MACHINES.includes(n)) return n;
...
async function pollAll() {
  // Parallel fetches.
  const [stats, peers, llm, nats, device, services, natsStats, ticker, security] = await Promise.allSettled([
    poll('/api/stats'),
    poll('/api/peers'),
    poll('/api/llm'),
    poll('/api/nats'),
  ...
  if (device.status === 'fulfilled' && device.value && device.value.battery) {
    updateDeviceTile(device.value);
  }
...
async function init() {
  ...
  try {
    ...
    MACHINES = (data.machines || []).map(m => m.id);
    for (const m of (data.machines || [])) {
      DISPLAY_NAMES[m.id] = (m.display_name || m.id).toUpperCase();
      if (m.wide) WIDE_TILES.push(m.id);
    }
  } catch (e) {
    console.warn('Failed to fetch /api/machines, using empty list');
  }
