...
// MACHINES is populated from /api/machines on init.
let MACHINES = [];
let DISPLAY_NAMES = {};
let WIDE_TILES = [];
...
const LOGOS = {
  ...
  linux: `<svg viewBox="0 0 24 24"><path fill="#FCC624" d="M12.504 0c-.155 0-.315.008-.48.021-4.226.333-3.105 4.807-3.17 6.298-.076 1.092-.3 1.953-1.05 3.02-.885 1.051-2.127 2.75-2.716 4.521-.278.832-.41 1.684-.287 2.489a.424.424 0 00-.11.135c-.26.268-.45.6-.663.839-.199.199-.485.267-.797.4-.313.136-.658.269-.864.68-.09.189-.136.394-.132.602 0 .199.027.4.055.536.058.399.116.728.04.97-.249.68-.28 1.145-.106 1.484.174.334.535.47.94.601.81.2 1.91.135 2.774.6.926.466 1.866.67 2.616.47.526-.116.97-.464 1.208-.946.587.26 1.24.43 1.922.512 1.966.238 4.306-.14 4.703-2.368l.012-.01c.065-.267.12-.552.167-.869.128-.834-.08-1.82-.327-2.735-.59-2.199-1.82-3.892-2.677-4.952-.726-1.064-.95-1.895-1.026-2.985-.064-1.482 1.062-5.97-3.069-6.3A5.88 5.88 0 0012.504 0z"/></svg>`,
};
...
// LLM tile is attached to the first machine that has an LLM section, or the first machine.
let llmTileId = null;

function updateLLM(data) {
  // Attach LLM section to the tile identified by data.machine, or first MACHINES entry.
  const targetId = data.machine || (llmTileId ? llmTileId : (MACHINES.length > 0 ? MACHINES[0] : null));
  if (!targetId) return;
  llmTileId = targetId;
  const tile = document.getElementById('t-' + targetId);
  if (!tile) return;
  ...
  if (!el) {
    ...
    const agents = document.getElementById('ta-' + targetId);
...
// Device tile is attached to the machine that reports device data (battery, power, etc).
let deviceTileId = null;

function updateDeviceTile(d) {
  // Attach to the machine identified by d.machine, or fallback.
  const targetId = d.machine || deviceTileId || (MACHINES.length > 0 ? MACHINES[MACHINES.length - 1] : null);
  if (!targetId) return;
  deviceTileId = targetId;
  const tile = document.getElementById('t-' + targetId);
  if (!tile) return;
