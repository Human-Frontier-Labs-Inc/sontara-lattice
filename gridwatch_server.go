...
func runGridwatch(ctx context.Context) error {
	...
	mux.HandleFunc("/api/device", gw.handleDevice)
	mux.HandleFunc("/api/machines", gw.handleMachines)
...
func (gw *Gridwatch) fetchPeers() {
	// Reload token from disk each fetch -- handles token rotation without restart.
	freshToken := loadAuthToken()
	if freshToken != "" {
	...
	if resp, err := client.Do(req); err == nil {
		...
		if resp.StatusCode == 200 {
			...
			// Auth failed or broker down -- clear cache so UI shows empty, not stale data.
			gw.peersCache = nil
		}
		gw.peersMu.Unlock()
	}
...
func (gw *Gridwatch) subscribeNATS(ctx context.Context) {
	...
	opts := natsConnectOptions("claude-peers-gridwatch")
...
func (gw *Gridwatch) handleDevice(w http.ResponseWriter, r *http.Request) {
	gw.natsMu.RLock()
	data := gw.deviceData
	gw.natsMu.RUnlock()
	if data == nil {
...
func (gw *Gridwatch) handleMachines(w http.ResponseWriter, r *http.Request) {
	...
	data, _ := json.Marshal(map[string]any{"machines": machines})
	jsonResponse(w, data)
}
...
func (gw *Gridwatch) fetchSecurity() {
	...
	if resp.StatusCode == 200 {
		...
		gw.secCache = nil // Clear stale data on auth failure
	}
	gw.secMu.Unlock()
}
