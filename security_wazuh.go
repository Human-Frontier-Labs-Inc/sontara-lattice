...
func runWazuhBridge(ctx context.Context) error {
	...
	for {
		...
		case <-ctx.Done():
			return nil
		case alert := <-alerts:
			// Skip noise: level 1-2 are trivial, and rule 2501 causes a feedback
			// loop (Wazuh re-ingests our own log lines as "authentication failure").
			if alert.Rule.Level < 3 || alert.Rule.ID == "2501" {
				continue
			}
			...
			if pub.js != nil {
				if _, err := pub.js.Publish(subject, data); err != nil {
					log.Printf("[wazuh-bridge] publish failed: %v", err)
				}
			}

			// NOTE: Do NOT log alert descriptions here -- Wazuh monitors journald
			// and will re-ingest log lines containing security keywords, creating
			// a feedback loop that generates millions of false alerts.
			log.Printf("[wazuh-bridge] published type=%s machine=%s", event.Type, event.Machine)
		}
	}
}
