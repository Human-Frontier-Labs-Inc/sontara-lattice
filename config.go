...
type Config struct {
	...
	// FleetTargets maps machine names to SSH targets (hostname, alias, or IP).
	// Used by fleet SSH operations (deploy, reauth, sim-attack).
	FleetTargets map[string]string `json:"fleet_targets"`

	// MachineIPs maps machine names to their expected network IPs.
	// Used for soft IP-binding verification in the auth middleware.
	MachineIPs map[string][]string `json:"machine_ips"`

	// AlertEmail is the email address for security and daemon alerts.
...
// loadFleetTargets returns fleet SSH targets from config.
func loadFleetTargets() map[string]string {
	if cfg.FleetTargets != nil {
		return cfg.FleetTargets
	}
	return map[string]string{}
}

// loadMachineIPs returns machine IP mappings from config.
func loadMachineIPs() map[string][]string {
	if cfg.MachineIPs != nil {
		return cfg.MachineIPs
	}
	return map[string][]string{}
}
