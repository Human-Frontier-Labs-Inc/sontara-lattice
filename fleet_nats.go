...
func newNATSPublisher() *NATSPublisher {
	...
	streamCfg := &nats.StreamConfig{
		...
		MaxAge:            24 * time.Hour,
		MaxBytes:          256 * 1024 * 1024, // 256MB -- sufficient after feedback loop fix
		MaxMsgsPerSubject: 10000,             // Prevent any single subject from dominating
...
func subscribeFleet(consumerName string, handler func(FleetEvent), opts ...nats.SubOpt) (*nats.Conn, error) {
	...
	// DeliverAll would replay the entire stream on first creation, which is almost never wanted.
