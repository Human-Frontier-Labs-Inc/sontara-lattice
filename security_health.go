...
func (b *Broker) subscribeSecurityEvents(ctx context.Context) {
	opts := natsConnectOptions("claude-peers-security-monitor")
	...
	_, err = js.Subscribe("fleet.security.>", func(msg *nats.Msg) {
		...
		// NOTE: Do NOT log alert descriptions -- Wazuh monitors journald and
		// re-ingests lines with security keywords, creating a feedback loop.
		log.Printf("[security] event machine=%s type=%s", event.Machine, event.Type)
