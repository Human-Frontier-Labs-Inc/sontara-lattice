...
No deploy.conf found. Create one with your fleet hosts:
  ...
  my-server:user@server-ip:claude-peers-linux-amd64
  my-pi:user@pi-ip:claude-peers-linux-arm64
  my-mac:user@mac-ip:claude-peers-darwin-arm64

Broker host (for restart after deploy):
  # Set BROKER_SSH to the broker's ssh target
  BROKER_SSH=user@server-ip
EOF
  exit 1
fi
