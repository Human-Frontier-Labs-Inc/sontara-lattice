...
func buildFleetMemory(peers []Peer, events []Event, health map[string]*MachineHealth) string {
	...
	if len(events) == 0 {
		...
		for _, e := range events {
			...
			seen[key] = true
			// Skip repetitive summary_changed events (dedup key already covers peer+type,
			// so this only removes the first occurrence of noisy event types).
...
func shortenPath(p string) string {
	...
	// Collapse any /home/<user>/ or /Users/<user>/ to ~/
	if idx := strings.Index(p, "/home/"); idx == 0 {
		parts := strings.SplitN(p[6:], "/", 2)
		if len(parts) == 2 {
			return "~/" + parts[1]
		}
		return "~"
	}
	if idx := strings.Index(p, "/Users/"); idx == 0 {
		parts := strings.SplitN(p[7:], "/", 2)
		if len(parts) == 2 {
			return "~/" + parts[1]
		}
		return "~"
