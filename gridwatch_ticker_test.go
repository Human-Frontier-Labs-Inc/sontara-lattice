...
func TestTickerBusPushAndEvents(t *testing.T) {
	...
	tb.Push("disk", "error", "server-1 disk 92%", "")
	tb.Push("peer", "info", "laptop-1 joined", "")
...
func TestTickerHandler(t *testing.T) {
	...
	gw.ticker.Push("disk", "error", "server-1 disk 91%", "")
	...
	// Newest first.
	if events[0].Title != "server-1 disk 91%" {
		t.Errorf("events[0].Title = %q, want 'server-1 disk 91%%'", events[0].Title)
	}
}
