...
func TestGridwatchHandlers(t *testing.T) {
	...
	// Test /api/device (empty)
	rec = httptest.NewRecorder()
	gw.handleDevice(rec, httptest.NewRequest("GET", "/api/device", nil))
	if rec.Code != http.StatusOK {
		t.Errorf("device: status = %d", rec.Code)
	}
	if rec.Body.String() != "{}" {
		t.Errorf("device: expected empty object, got %s", rec.Body.String())
	}
}
...
func TestLoadGridwatchConfigFromFile(t *testing.T) {
	dir := t.TempDir()
	configFile := filepath.Join(dir, "gridwatch.json")
	os.WriteFile(configFile, []byte(`{"port":9999,"machines":[{"id":"test","os":"arch","host":"10.0.0.1"}]}`), 0644)
	t.Setenv("GRIDWATCH_CONFIG", configFile)

	gwc := loadGridwatchConfig()
	if gwc.Port != 9999 {
		t.Errorf("port = %d, want 9999", gwc.Port)
	}
	if len(gwc.Machines) != 1 {
		t.Fatalf("machines = %d, want 1", len(gwc.Machines))
	}
}

func TestLoadGridwatchConfigPortEnvOverride(t *testing.T) {
	dir := t.TempDir()
	configFile := filepath.Join(dir, "gridwatch.json")
	os.WriteFile(configFile, []byte(`{"port":8888}`), 0644)
	t.Setenv("GRIDWATCH_CONFIG", configFile)
	t.Setenv("GRIDWATCH_PORT", "7777")
	gwc := loadGridwatchConfig()
	if gwc.Port != 7777 {
		t.Errorf("port = %d, want 7777", gwc.Port)
	}
}
...
func TestHandleDaemons(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	gw.handleDaemons(rec, httptest.NewRequest("GET", "/api/daemons", nil))
	if rec.Code != http.StatusOK {
...
func TestHandleServices(t *testing.T) {
	...
	// Nil cache.
	rec := httptest.NewRecorder()
	gw.handleServices(rec, httptest.NewRequest("GET", "/api/services", nil))
	if rec.Code != http.StatusOK {
		t.Errorf("services: status = %d", rec.Code)
	}
	if rec.Body.String() != "{}" {
		t.Errorf("services: expected empty object, got %s", rec.Body.String())
	}
}
...
func TestHandleSecurity(t *testing.T) {
	...
	// Nil cache.
	rec := httptest.NewRecorder()
	gw.handleSecurity(rec, httptest.NewRequest("GET", "/api/security", nil))
	if rec.Code != http.StatusOK {
		t.Errorf("security: status = %d", rec.Code)
	}
	if rec.Body.String() != "{}" {
		t.Errorf("security: expected empty object, got %s", rec.Body.String())
	}
}
