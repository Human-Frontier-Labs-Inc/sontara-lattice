...
// sshExec runs a command on a remote machine via SSH using the shared helper.
...
func (rd *ResponseDaemon) captureForensics(machine string, extraFile string) (*ForensicSnapshot, error) {
	...
	// LD_PRELOAD hijack check
	snap.LDPreload, _ = rd.sshExec(machine, "env | grep -i ld_preload 2>/dev/null || true")

	// Kernel modules
	if !isMAC {
		snap.KernelMods, _ = rd.sshExec(machine, "lsmod | head -20")
	}

	// Preload file
	if !isMAC {
		snap.PreloadFile, _ = rd.sshExec(machine, "cat /etc/ld.so.preload 2>/dev/null || true")
	}

	// PAM modules
	if !isMAC {
		snap.PAMModules, _ = rd.sshExec(machine, "ls -la /etc/pam.d/ 2>/dev/null || true")
	}

	// User crontab
	snap.UserCrontab, _ = rd.sshExec(machine, "crontab -l 2>/dev/null || true")

	// System cron
	if !isMAC {
		snap.SystemCron, _ = rd.sshExec(machine, "ls /etc/cron.d/ 2>/dev/null || true")
	} else {
		snap.SystemCron, _ = rd.sshExec(machine, "ls /usr/lib/cron/tabs/ 2>/dev/null || true")
	}

	// Hosts file (non-comment, non-empty lines)
	snap.HostsFile, _ = rd.sshExec(machine, "cat /etc/hosts 2>/dev/null | grep -v '^#' | grep -v '^$' || true")
