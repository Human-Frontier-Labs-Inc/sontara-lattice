...
// sendEmailAlert sends a security alert email, throttled to 1 per machine per 15 minutes.
func (sw *SecurityWatch) sendEmailAlert(machine string, event SecurityEvent, reason string) {
	sw.mu.Lock()
	if lastSent, ok := sw.emailThrottle[machine]; ok && time.Since(lastSent) < 15*time.Minute {
		sw.mu.Unlock()
		return
	}
	...
	if cfg.AlertEmail != "" {
		sendEmail(cfg.AlertEmail, subject, body)
	}
}
